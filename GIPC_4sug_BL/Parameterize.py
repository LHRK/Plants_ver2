...
def map_aa2cg (u, resname, bead_assignments, bead_names, bead_sizes, AA_traj, GRO):
    ...
    with open('/mnt/f/AA_SIMS_WATER_IONS/Amine2_0/CG_WAT/CG_SASA/vdwradii.dat') as filein:
        vdw = filein.readlines()
    vdw = vdw[:15]
    for idx, beadname in enumerate(bead_names):
        #vdw.append(f'???  {beadname}   {sizedict[bead_sizes[idx]]}\n')
        vdw.append('???  {0}   {1}\n'.format(beadname, sizedict[bead_sizes[idx]]))
    ...
    os.system('cp /mnt/f/AA_SIMS_WATER_IONS/Amine2_0/AA/vdwradii.dat .')
...
def run_SASA(name, target, resname, dir_out, bead_names, bead_sizes, selection='all'):
    ...
    if name == 'AA':
        tgt = u.select_atoms('resname {}'.format(resname)).residues[0].atoms.select_atoms(selection)
    ...
    tgt.atoms.write("{}/gro.gro".format(dir_writing))

    ## Calculate SASA & connoly surface
    os.chdir(dir_writing)
    print ('entering {}'.format(dir_writing))
    p = subprocess.Popen("gmx sasa -f ../{0} -s ../{1} -n {2}/index.ndx -ndots 4800 -probe 0.191 -or {2}/resarea_SASA.xvg -o {2}/SASA.xvg ".format(xtc, gro, dir_writing )
                , stdin=subprocess.PIPE, shell=True, universal_newlines=True)
    p.communicate('TGT')
    p.wait()

    p = subprocess.Popen("gmx sasa -s gro.gro -o temp.xvg -probe 0.191 -ndots 240 -q surface.pdb"
            , stdin=subprocess.PIPE, shell=True, universal_newlines=True)
...
def run_SASA_old(name, target, resname, dir_out, bead_names, bead_sizes):
    ''''Give name of simulation, AA for atomistic, resname in the gro file, and an out_put directory, in here folders will be made'''
    try:
        dir_writing = '{}/{}'.format(dir_out, name)
        os.mkdir(dir_writing)
    ...
    tgt.write("{}/index.ndx".format(dir_writing), mode="w", name= 'TGT')
    #tgt.atoms.write(f"{dir_writing}/gro.gro")
    tgt.atoms.write("{0}/gro.gro".format(dir_writing))
...
def prepare_setup_BL(itp_loc, mdp_loc, lipids='POPC DPPC DIPC DPSM CHOL DBPC DOPC'):
    ...
    itpDir='/mnt/f/AA_SIMS_WATER_IONS/MDPs'
    itpdir2 = '/home/ltr/PYTHON/M3-Lipid-Parameters/openbeta'
    IL_tails = '/home/ltr/PYTHON/M3-Ionizable-Lipids_dev/Collection_of_itps/Martini_v1_tails-2.itp'
    ...
    top[0]= '#include "{0}/martini_v3.0.0.itp"\n#include "{0}/martini_v3.0.0_ffbonded_v2_openbeta.itp"\n#include "{1}"\n#include "{0}/martini_v3.0.0_phospholipids_PC_v2_openbeta.itp"\n#include "{0}/martini_v3.0.0_solvents_v1.itp"\n#include "{0}/martini_v3.0.0_ions_v1.itp"\n'.format(itpdir2, itp_loc)
    ...
    subprocess.call('gmx grompp -f {}/min.mdp -c mem.gro -p topol.top -o memion.tpr -maxwarn 5'.format(mdp_loc)
                , shell=True)
    ...
    subprocess.call('gmx grompp -f {}/min.mdp -c memion.gro -p topol.top -o memion_2.tpr -maxwarn 5'.format(mdp_loc)
                , shell=True)
    ...
    ### Relax the system
    subprocess.call("gmx grompp -f {}/rel_310.mdp -c m.gro -p topol.top -o r.tpr -n index.ndx -maxwarn 5".format(mdp_loc)
                    , shell = True)
    subprocess.call("gmx mdrun -v -deffnm r -nt 5 -rdd 1.6"
                , shell = True)
    
    ### Prepare the simulation
    subprocess.call("gmx grompp -f {}/prod_310.mdp -c r.gro -p topol.top -o p.tpr -n index.ndx -maxwarn 5".format( mdp_loc)
                    , shell = True)
    ### Prepare the simulation
    subprocess.call("gmx mdrun -v -deffnm p  -nt 4 -rdd 1.6".format(mdp_loc)
                    , shell = True)
    return
...
def prepare_setup_water(mdp_loc):
    ...
    ###
    ### gmx solvate
    subprocess.call('gmx solvate -cp box.gro -cs {0}/water.gro -o watered.gro -p topol.top'.format(mdp_loc)
                , shell=True)
    ...
    subprocess.call('gmx grompp -f {}/min.mdp -c watered.gro -p topol.top -o memion.tpr -maxwarn 1'.format(mdp_loc)
                , shell=True)
    #print ('second grompp : gmx grompp -f {}/min.mdp -c watered.gro -p topol.top -o memion.tpr -maxwarn 1'.format(mdp_loc))
    p = subprocess.Popen('gmx genion -s memion.tpr -o memion.gro -p topol.top -pname NA -pq +1 -nname CL -nq -1 -neutral'
                         , stdin=subprocess.PIPE, shell=True, universal_newlines=True)
    p.communicate('W')
    p.wait()
...
def prepare_setup_water_DEV(mdp_loc):
    '''Function to setup and run simulation of the molecule in water'''
    u=md.Universe('cg_good.gro', '../cg.xtc')
    sel =u.select_atoms('all')
    resname = np.unique(sel.resnames)[0]
